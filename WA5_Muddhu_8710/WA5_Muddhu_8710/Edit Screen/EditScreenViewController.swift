...
class EditScreenViewController: UIViewController {
    ...
    var delegate:DetailsScreenViewController!
    ...
    func populateFields() {
        if let contact = contact {
            ...
            editContactScreen.textFieldPhoneNumber.text = contact.phoneNumber
            editContactScreen.textFieldAddress.text = contact.address
            editContactScreen.textFieldCityState.text = contact.stateCity
            editContactScreen.textFieldZip.text = contact.zip
            editContactScreen.buttonSelectType.setTitle(contact.type, for: .normal)
            
            // Set the picked image if available
            if let image = contact.image {
                editContactScreen.buttonTakePhoto.setImage(image.withRenderingMode(.alwaysOriginal), for: .normal)
                pickedImage = image
            } else {
                let defaultImage = UIImage(systemName: "person.fill")
                editContactScreen.buttonTakePhoto.setImage(defaultImage?.withTintColor(.black, renderingMode: .alwaysOriginal), for: .normal)
            }
        }
    }
    ...
    @objc func onSaveButtonTapped(){
        ...
        let phoneNumber = editContactScreen.textFieldPhoneNumber.text ?? ""
        let address = editContactScreen.textFieldAddress.text ?? ""
        let cityState = editContactScreen.textFieldCityState.text ?? ""
        let zip = editContactScreen.textFieldZip.text ?? ""
        ...
        let updatedContact = Contact(name: name, email: email, phoneNumber: phoneNumber, type: selectedType, address: address, stateCity: cityState, zip: zip, image: pickedImage ?? UIImage(systemName: "person.fill")?.withTintColor(.black, renderingMode: .alwaysOriginal))
        delegate.delegateOnEditContact(contact: updatedContact, at: contactIndex!)
...
extension EditScreenViewController: UINavigationControllerDelegate, UIImagePickerControllerDelegate{
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let image = info[.editedImage] as? UIImage{
            self.editContactScreen.buttonTakePhoto.setImage(image.withRenderingMode(.alwaysOriginal), for: .normal)
            self.pickedImage = image
        }
        else{
            let personImage = UIImage(systemName: "person.fill")
            self.editContactScreen.buttonTakePhoto.setImage(personImage, for: .normal)
